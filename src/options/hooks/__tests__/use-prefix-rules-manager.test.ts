...
it("usePrefixRulesManager loads stored rules", async () => {
...
it("usePrefixRulesManager adds a normalized host rule and resets the draft", async () => {
  const { result } = renderHook(() => usePrefixRulesManager())
...
it("usePrefixRulesManager reports add validation errors", async () => {
  const { result } = renderHook(() => usePrefixRulesManager())

  await act(async () => result.current.addRule())
  expect(result.current.addDraft.error).toBe(
    "Pattern and prefix are required"
  )
...
it("usePrefixRulesManager reports duplicate add errors", async () => {
  storage.tegakariPrefixRules = [{ pattern: "a.com", prefix: "[a]" }]
  const { result } = renderHook(() => usePrefixRulesManager())
  await waitFor(() => expect(result.current.rules).toHaveLength(1))
...
it("usePrefixRulesManager trims regex patterns and prefixes", async () => {
  const { result } = renderHook(() => usePrefixRulesManager())
  await waitFor(() => expect(result.current.rules).toEqual([]))
...
it("usePrefixRulesManager edits, reorders, deletes, and cancels rules", async () => {
...
it("usePrefixRulesManager reports duplicate edit errors", async () => {
  storage.tegakariPrefixRules = [
    { pattern: "a.com", prefix: "[a]" },
    { pattern: "b.com", prefix: "[b]" },
  ]
