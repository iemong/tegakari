...
以下はMarkdown/JSONLのフル出力形式の例です。出力プリセット（Claude Code / Cursor / Minimal）、カスタムテンプレート、クイック指示チップ（タグ）、スタイル変更差分、CSS出所情報、リレーションについては [`docs/output-spec.md`](docs/output-spec.md) を参照してください。
